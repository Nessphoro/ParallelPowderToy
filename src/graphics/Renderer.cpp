...
#pragma omp parallel for private(i,j,x,y,r,g,b,nx,ny) num_threads(4) schedule(static)
...
#pragma omp parallel for private(x,y,i,j,c) num_threads(4) schedule(static)
	for (x=0; x<CELL; x++)
		for (y=0; y<CELL; y++)
			for (i=-CELL; i<CELL; i++)
				for (j=-CELL; j<CELL; j++)
...
#endif
#pragma omp parallel for private(deca, decr, decg, decb, cola, colr, colg, colb, firea, firer, fireg, fireb, pixel_mode, q, i, t, nx, ny, x, y, caddress) num_threads(4) schedule(static)
